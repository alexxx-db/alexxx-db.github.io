  ...
  var SyntaxHighlighter = (function () {
    ...
    var sh = {
      ...
      config: {
        ...
        strings: {
          ...
          aboutDialog:
            '<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd"><html xmlns="http://www.w3.org/1999/xhtml"><head><meta http-equiv="Content-Type" content="text/html; charset=utf-8" /><title>About SyntaxHighlighter</title></head><body style="font-family:Georgia,"Times New Roman",Times,serif;background-color:#fff;color:#000;font-size:1em;text-align:center;"><div style="text-align:center;margin-top:3em;"><div style="font-family:Geneva,Arial,Helvetica,sans-serif;font-size:xx-large;">SyntaxHighlighter</div><div style="font-size:.75em;margin-bottom:4em;"><div>version 2.0.278 (February 03 2009)</div><div><a href="http://alexgorbatchev.com" target="_blank" style="color:#0099FF;text-decoration:none;">http://alexgorbatchev.com</a></div></div><div>JavaScript code syntax highlighter.</div><div>Copyright 2004-2009 Alex Gorbatchev.</div></div></body></html>',
        },
      ...
      toolbar: {
        ...
        createButton: function (label, highlighterId, commandName) {
          ...
          if (isNaN(width) == false) style.width = width + "px";

          if (isNaN(height) == false) style.height = height + "px";
        ...
        executeCommand: function (
          ...
          if (
            highlighter == null ||
            (command = highlighter.toolbarCommands[commandName]) == null
          )
            return null;
        ...
        items: {
          ...
          viewSource: function (highlighter) {
            ...
            this.execute = function (sender, event, args) {
              var code = sh.utils
                  .fixForBlogger(highlighter.originalCode)
                  .replace(/</g, "&lt;"),
                wnd = sh.utils.popup(
                  "",
                  "_blank",
            ...
            this.create = function () {
              ...
              function params(list) {
                ...
                for (var name in list)
                  result +=
                    "<param name='" + name + "' value='" + list[name] + "'/>";
            ...
            this.execute = function (sender, event, args) {
              ...
              switch (command) {
                case "get":
                  ...
                  else return sh.utils.unindent(code);
      ...
      utils: {
        ...
        eachLine: function (str, callback) {
          ...
          for (var i = 0; i < lines.length; i++) lines[i] = callback(lines[i]);
        ...
        decorate: function (str, css) {
          ...
          str = str.replace(/ {2,}/g, function (m) {
            ...
            for (var i = 0; i < m.length - 1; i++) spaces += "&nbsp;";
        ...
        fixForBlogger: function (str) {
          return sh.config.bloggerMode == true
        ...
        unindent: function (str) {
          ...
          for (var i = 0; i < lines.length && min > 0; i++) {
            ...
            if (sh.utils.trim(line).length == 0) continue;
    ...
    sh.Highlighter.prototype = {
      ...
      removeNestedMatches: function () {
        for (var i = 0; i < this.matches.length; i++)
          if (this.isMatchNested(this.matches[i])) this.matches[i] = null;
      },
      ...
      createDisplayLines: function (code) {
        ...
        for (var i = 0; i < lines.length; i++) {
          var line = lines[i],
            ...
            highlighted =
              highlightedLines.indexOf((firstLine + i).toString()) != -1,
            spaces = null;
          if (indent != null) {
          ...
          code +=
            ...
            (spaces != null
              ? '<code class="spaces">' +
                ...
                "</code>"
              : "") +
            '<span class="block" style="margin-left: ' +
            indent +
            'px;">' +
            line +
            "</span>" +
      ...
      processMatches: function (code, matches) {
        ...
        for (var i = 0; i < matches.length; i++) {
          ...
          result +=
            decorate(code.substr(pos, match.index - pos), "plain") +
      ...
      highlight: function (code, params) {
        ...
        if (vars.spaceWidth === null) vars.spaceWidth = sh.utils.measureSpace();
        ...
        if (this.getParam("collapse")) div.className += " collapsed";

        if (this.getParam("gutter") == false) div.className += " nogutter";

        div.className += " " + this.getParam("class-name");
        div.style.fontSize = this.getParam("font-size", ""); // IE7 can't take null

        this.originalCode = code;
        this.code = sh.utils.trimFirstAndLastLines(code).replace(/\r/g, " "); // IE lets these buggers through
        ...
        // add columns ruler
        if (this.getParam("ruler")) div.appendChild(sh.utils.createRuler());
        ...
        if (this.getParam("auto-links")) code = sh.utils.processUrls(code);
      ...
      while ((match = real.exec.call(part, pattern))) {
        ...
        if (match[2]) {
          ...
          if (pattern.charAt(part.lastIndex) === "]") {
            ...
            cc = XRegExp.matchRecursive(
              "&&" + pattern.slice(match.index),
              lib.classLeft,
              lib.classRight,
              "",
              { escapeChar: "\\" },
          ...
          if (singleline && match[7] === ".") {
            ...
            len = real.exec.call(
              lib.extended,
              pattern.slice(part.lastIndex - 1),
            )[0].length;
            // keep tokens separated unless the following token is a quantifier
...
XRegExp.cache = function (pattern, flags) {
  ...
  return (
    XRegExp.cache[key] || (XRegExp.cache[key] = new XRegExp(pattern, flags))
  );
};
...
XRegExp.matchRecursive = function (str, left, right, flags, options) {
    ...
    left =
      left instanceof RegExp
        ? left.global
          ? left
          : left.addFlags("g")
        : new XRegExp(left, "g" + flags),
    right =
      right instanceof RegExp
        ? right.global
          ? right
          : right.addFlags("g")
        : new XRegExp(right, "g" + flags),
    output = [],
    openTokens = 0,
    delimStart = 0,
    delimEnd = 0,
  ...
  if (escapeChar) {
    ...
    if (multiline)
      throw TypeError(
        "can't supply escape character when using the multiline flag",
      );
